    ...
    <div class="articleListWrap">
      ...
      <div class="articleList">
        <div class="cell" v-for="list in articleList" :key="list.id">
          ...
          <router-link class="list_title" :to="{name:'article',params:{id:list.id,name:list.author.loginname}}">{{list.title}}</router-link>
...
export default {
  ...
  methods: {
    ...
    getData(){
      this.$http
              .get(`https://cnodejs.org/api/v1/topics?number=1&limit=40`)
              .then(result => result.data.data)
              .then(articleList => {
                this.articleList = articleList;
              });
...
.articleListWrap {
  ...
  .list_type {
    ...
    .type > a {margin-left: 10px;margin-right: 10px;font-size: 14px;padding: 3px 4px;color: #80bd01;}
    .active > a {background-color: #80bd01;color: #fff;border-radius: 3px;}
  }
  .articleList {
    ...
    .cell {
      ...
      .cell_time {margin-left: auto;margin-right: 10px;padding-left: 30px;color: #778087;font-size: 13px;white-space: nowrap;}
      .list_info_number {
        width: 70px;
        padding: 0 5px;
        .reply_count {color: #9e78c0;font-size: 14px;}
        .like_count {color: #b4b4b4;font-size: 10px;}
      }

      .list_tag {background-color: #e5e5e5;font-size: 12px;color: #999;padding: 2px 4px;border-radius: 3px;white-space: nowrap;}
      .special {background-color: #80bd01;color: white;}
      .list_title {
        overflow: hidden;
    ...
    .cell > a > img {width: 30px;height: 30px;border-radius: 3px;}
  }
}
</style>
